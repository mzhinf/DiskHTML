...
4. 等待状态栏显示“HTML 快照已生成”。

生成后会得到同名的 `.html` 和 `.sqlite3` 两个文件。HTML 是可直接浏览的离线页面，左侧提供可展开的层级文件树；单击目录即可筛选右侧文件清单。每个文件均显示 SHA-256、大小、修改时间、创建时间和错误信息。SQLite 是可再渲染的快照索引，可用于生成以后版本的 HTML，而不必重新扫描原始目录。

## 在命令提示符生成快照

重新构建后的 DiskHTML.exe 也支持 HTML 快照命令。进入 EXE 所在目录后，在命令提示符中可执行：
...
命令成功后会在当前目录生成 `资料快照.html` 和 `资料快照.sqlite3`。两者均不能已存在。

## 从 SQLite 重新生成 HTML

当页面布局升级时，不必重新扫描原目录。点击“从 SQLite 生成 HTML”，选择同名 `.sqlite3` 快照索引并指定一个新的 `.html` 输出文件即可。
...
1. 点击“比较快照目录”。
2. 选择历史 HTML 快照。
3. 在弹出的文件树中选择要比较的历史目录；“快照根目录”表示整个快照。
4. 选择任意本机目录。
5. 指定新的 .html 比较报告。
...
默认不跟随软链接和 Windows 重解析目录。如需将链接指向的文件写入快照，在 `snapshot` 或 `compare-source` 后使用 `--follow-links`：
