...
首次启动会从 `_internal\config\config.example.toml` 复制生成 EXE 同目录的 `config.toml`。程序默认读取该文件，后续启动不会覆盖用户修改；配置损坏或缺少内置模板时会明确失败，避免静默使用不同默认值。
...
HTML 类似桌面文件浏览器，可离线查看目录树、文件列表、大小、时间、摘要及具体算法；SQLite 保留结构化扫描数据，可由同版本程序重新生成 HTML。

“Hash 策略”默认选择完整 SHA-256。也可选择采样模式并设置目标采样读取量和固定采样次数；只有超过目标读取量的大文件使用采样指纹。采样指纹仅用于快速预检，不是完整内容一致性证明。
...
历史目录会重定根，因此两侧绝对路径、盘符和根目录名可以不同。程序自动读取 HTML 指定的 Hash 策略，并用同一策略扫描当前目录，不提供另一个算法选择。报告区分 `MATCH`、`PRECHECK_MATCH`、`CHANGED`、`ADDED`、`MISSING` 和 `ERROR`；详细含义见 [DiskHTML.exe 使用指南](diskhtml-exe-guide.md)。

## 从 SQLite 重新生成

在“从 SQLite 生成”页选择快照同名 `.sqlite3` 和新的 HTML 路径。该过程读取保存的数据，不重新访问或 Hash 原目录。旧版本数据库不升级；格式不兼容时需重新扫描生成。
