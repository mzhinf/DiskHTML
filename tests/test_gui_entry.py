...
class GuiEntryTests(TestCase):
    ...
    def test_arguments_dispatch_to_command_entry(self) -> None:
        ...
        with (
            ...
            self.assertEqual(entry.main(["backup", "F:\\Documents", ".\\资料冷备.html"]), 0)

        command_main.assert_called_once_with(["backup", "F:\\Documents", ".\\资料冷备.html"])
        gui_main.assert_not_called()

    def test_backup_argument_generates_html_through_entry(self) -> None:
        """打包入口的 backup 参数应实际生成 HTML，而不是启动 GUI。"""
        ...
        with TemporaryDirectory(dir=Path(__file__).parent) as directory:
            ...
            self.assertEqual(_load_entry_module().main(["backup", str(source), str(output)]), 0)
