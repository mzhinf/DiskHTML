...
HTML 是离线浏览页面；SQLite 是同格式版本的可再渲染索引。每个文件记录 Name、Size、Modified、Created、摘要和算法。采样指纹只用于快速预检，不是完整内容一致性证明。
...
2. 在“基准快照”选择以前生成的 DiskHTML HTML。
   页面会显示 HTML 指定的 Hash 策略，当前目录自动使用同一策略。
...
| 状态 | 含义 |
|---|---|
| `MATCH` | 大小、完整 SHA-256 和算法相同，即完整一致 |
| `PRECHECK_MATCH` | 大小、采样指纹和采样算法相同，仅表示采样预检一致 |
| `CHANGED` | 同一路径的大小、算法或摘要不同，即已变化 |
| `ADDED` | 仅存在于当前本机目录 |
| `MISSING` | 仅存在于基准快照 |
