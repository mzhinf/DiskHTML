...
class HtmlArchiveTests(TestCase):
    ...
    def test_backup_and_compare_are_visual_single_html_files(self) -> None:
        ...
        self.assertIn(".tree-children>.tree-node::before", archive_text)
        self.assertIn(".tree-node:not(:last-child)::after", archive_text)
        self.assertIn(".tree-branch{visibility:hidden}", archive_text)
        self.assertIn("margin-left:17px;padding-left:12px", archive_text)
